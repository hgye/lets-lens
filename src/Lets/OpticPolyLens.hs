...
mapL k = Lens r
  where r f m = case Map.lookup k m of
          -- couldn't find key
          Nothing -> (\x -> case x of
                         -- Nothing
...
setL k = Lens r
  where r f s = case Set.member k s of
...
choice l1 l2 = Lens r
  where r f x = case x of
          Left s -> pure Left <*> set l1 s <$> f (get l1 s)
          Right q -> pure Right <*> set l2 q <$> f (get l2 q)
  --error "todo: choice"
